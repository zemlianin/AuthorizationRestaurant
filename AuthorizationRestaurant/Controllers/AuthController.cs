...
namespace AuthorizationRestaurant.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register(RegistrationRequest request)
        {
            ...
            var result = await _userManager.CreateAsync(
                new User { UserName = request.Username, Email = request.Email, Role = request.Role, CreatedDate = DateTime.UtcNow, UpdatedDate = DateTime.UtcNow },
                request.Password
            );
            if (result.Succeeded)
            {
